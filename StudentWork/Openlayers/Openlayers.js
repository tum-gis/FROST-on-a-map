...
var measurementID2;
var zoomLevel = 10;
var cent = ol.proj.fromLonLat([11.575, 48.137]);
// Color Marker + Lines:
var getColorClass1;
var getColorClass2;
var getColorClass3;
...
if (thingIotID == 15) { //Markus Arduino
  ...
  cent = ol.proj.fromLonLat([11.517, 48.1855]);
  zoomLevel = 17;
  // Color Marker + Lines:
...
if (thingIotID == 16) { //Smartphone Robin
  ...
  cent = ol.proj.fromLonLat([11.473, 48.1855]);
  zoomLevel = 17;
...
if (thingIotID == 17) { //Smartphone Yifan
  ...
  cent = ol.proj.fromLonLat([114.36, 30.53]);
  zoomLevel = 13;
...
if (thingIotID == 19) { //Tobi's Arduino
  ...
  cent = ol.proj.fromLonLat([11.5154, 48.5309]);
  zoomLevel = 16;
...
// Map content
//#####################
// FROST server, Thing, time filter (optional)
...
fetch(url)
  ...
  .then(function (data) {
    ...
    data.features.forEach(function (feature) {
      ...
      latlngs.push(coords);
      latitude.push(Math.round(coords[0] * 100000) / 100000);
      longitude.push(Math.round(coords[1] * 100000) / 100000);
    ...
    fetch(url_1).then((resp) => resp.json()).then(function (data) {
      ...
      fetch(url_2).then((resp) => resp.json()).then(function (data) {
        ...
        // coordinates and lux&mic of polylines
        var latlon = [];
        ...
        for (var i = 0; i <= timestamp.length - 1; i++) {
          ...
          // For Polyline-Popup -> mean
          var colMea = (measurement1[i] + measurement1[i + 1]) / 2;

          // Polyline
          latlon[0] = ol.proj.fromLonLat([longitude[i], latitude[i]]);
          latlon[1] = ol.proj.fromLonLat([longitude[i + 1], latitude[i + 1]]);
          ...
          var layerLines = new ol.layer.Vector({
            source: new ol.source.Vector({
              features: [new ol.Feature({
                geometry: new ol.geom.LineString(latlonreverse),
              })]
            }),
          ...
          var markerContent =
            "<h4><a> Name: " + sensorName + " (Thing #" + thingIotID + ")</a></h4>" +
            "<b> Coordinates: </b>" + latitude[i] + "° , " + longitude[i] + "°" + "<br>" +
